...
const handleLogin = async () => {
  ...
  try {
    ...
    if (response.data.status === 'success') {
      authService.login(response.data.data.token)
      router.push('/admin')
    } else {
  ...
  <div class="min-h-[60vh] flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-gray-50">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      ...
      <p class="mt-2 text-center text-sm text-gray-600">
        {{ t('auth.login_sub', 'Sign in to access settings and data management') }}
      </p>
    </div>
        ...
        <form class="space-y-6" @submit.prevent="handleLogin">
          ...
          <div>
            ...
            >
              ...
              {{ loading ? t('common.loading', 'Loading...') : t('auth.sign_in', 'Sign In') }}
            </button>
          </div>
        </form>
      </div>
